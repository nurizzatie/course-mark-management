      ...
      <table class="table table-bordered align-middle text-center">
        ...
        <tbody>
          <tr v-for="(req, i) in requests" :key="req.id">
            <td>{{ i + 1 }}</td>
            <td>
              {{ req.student_name }}<br /><small>{{ req.matric_number }}</small>
            </td>
            ...
            <td>
              <a
                v-if="req.supporting_link"
                :href="getFileUrl(req.supporting_link)"
                target="_blank"
              >
                View
              </a>
...
export default {
  ...
  methods: {
    ...
    respond(id, status) {
      api
        .put(`/lecturer/remark-requests/${id}`, { status })
